...
namespace Ignis {
    void PBR::initMaterials(const uint32_t max_binding_count) {
    ...
    PBR::TextureID PBR::addTexture(const Vulkan::Image &image, vk::ImageView view) {
        TextureID id{};
    ...
    PBR::MaterialID PBR::addMaterial(const Material &material) {
        MaterialID id{};
    ...
    void PBR::removeTextureRC(const TextureID id) {
    ...
    void PBR::removeMaterialRC(const MaterialID id) {
    ...
    PBR::Material PBR::getMaterial(const MaterialID id) const {
        DIGNIS_ASSERT(m_Materials.contains(id));
    ...
    void PBR::readMaterialImages(FrameGraph::RenderPass &render_pass) {
        render_pass.readImages(m_FrameGraphImages.getData());
    }

    void PBR::readMaterialBuffers(FrameGraph::RenderPass &render_pass) const {
