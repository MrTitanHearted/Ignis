...
namespace Ignis {
    void PBR::initSkybox(const std::array<std::filesystem::path, 6> &skybox_face_paths) {
        ...
        const vk::ShaderModule skybox_shader_module = Vulkan::CreateShaderModuleFromSPV(skybox_shader_code);

        m_SkyboxPipeline =
            Vulkan::GraphicsPipelineBuilder()
                .setVertexShader("vs_main", skybox_shader_module)
                .setFragmentShader("fs_main", skybox_shader_module)
        ...
        Vulkan::DestroyShaderModule(skybox_shader_module);
        ...
        {
            const TextureAsset texture_asset = TextureAsset::LoadFromPath(skybox_face_paths[0], TextureAsset::Type::eRGBA8u).value();
...

        {
            const Vulkan::Buffer staging = Vulkan::AllocateBuffer(
                ...
                m_SkyboxVertexBuffer.Size +
                    ...
                    m_SkyboxImage.Extent.width *
                        m_SkyboxImage.Extent.height *
                        sizeof(uint8_t) * 4 *
                        6,
...

            {
                ...
                for (const std::filesystem::path &path : skybox_face_paths) {
                    TextureAsset texture_asset = TextureAsset::LoadFromPath(path, TextureAsset::Type::eRGBA8u).value();
        ...
        m_FrameGraphSkyboxVertexBuffer = FrameGraph::BufferInfo{
            m_FrameGraph.importBuffer(m_SkyboxVertexBuffer.Handle, m_SkyboxVertexBuffer.Usage, 0, m_SkyboxVertexBuffer.Size),
            0,
            m_SkyboxVertexBuffer.Size,
        ...
        m_FrameGraphSkyboxIndexBuffer = FrameGraph::BufferInfo{
            m_FrameGraph.importBuffer(m_SkyboxIndexBuffer.Handle, m_SkyboxIndexBuffer.Usage, 0, m_SkyboxIndexBuffer.Size),
            0,
            m_SkyboxIndexBuffer.Size,
    ...
    void PBR::releaseSkybox() {
        m_FrameGraph.removeBuffer(m_FrameGraphSkyboxVertexBuffer.Buffer);
        m_FrameGraph.removeBuffer(m_FrameGraphSkyboxIndexBuffer.Buffer);
        m_FrameGraph.removeImage(m_FrameGraphSkyboxImage);
    ...
    void PBR::readSkyboxImage(FrameGraph::RenderPass &render_pass) const {
        render_pass.readImages(m_FrameGraphSkyboxImage);
    }

    void PBR::readSkyboxBuffers(FrameGraph::RenderPass &render_pass) const {
        render_pass.readBuffers({m_FrameGraphSkyboxVertexBuffer, m_FrameGraphSkyboxIndexBuffer});
    }

    void PBR::onSkyboxDraw(const vk::CommandBuffer command_buffer) {
